...
class FireTest(testutils.BaseTestCase):
  ...
  def testBoolParsingWithNo(self):
    ...
    def fn1(cookie_stand, nothing):
      return cookie_stand, nothing

    self.assertEqual(fire.Fire(fn1, command=['--cookie_stand', '--nothing']),
                     (True, True))
    self.assertEqual(fire.Fire(fn1, command=['--cookie_stand', '--nonothing']),
                     (True, False))

    with self.assertRaisesFireExit(2):
      # In this case nothing=False (since rightmost setting of a flag gets
      # precedence), but it errors because cookie_stand has no value.
      fire.Fire(fn1, command=['--nothing', '--nonothing'])

    # In these examples, --nothing sets cookie_stand=False:
    def fn2(cookie_stand, **kwargs):
      return cookie_stand, kwargs
    self.assertEqual(fire.Fire(fn2, command=['--cookie_stand']), (True, {}))
    self.assertEqual(fire.Fire(fn2, command=['--nothing']), (False, {}))
    with self.assertRaisesFireExit(2):
      # In this case, nothing=True, but it errors because cookie_stand has no value.
    ...
    def fn3(arg, **kwargs):
      return arg, kwargs
    self.assertEqual(fire.Fire(fn3, command=['--arg=value', '--cookie_stand']),
                     ('value', {'cookie_stand': True}))
    self.assertEqual(fire.Fire(fn3, command=['--arg=value', '--nothing']),
                     ('value', {'cookie_stand': False}))
